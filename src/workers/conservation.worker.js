...
export const calculateConservation = (
  ...
  const finalSampleSize= sampleSize ? Math.min(sampleSize, sequences.length): sequences.length;

  self.conservation = Array.from(Array(length).keys()).map(() => ({}));
  let n = 0;
  for (let seq of sampleSize ? sequences.slice(0, finalSampleSize) : sequences) {
